...
- DOCX 保存前在同目录生成滚动备份 `<文件名>.wenshu.bak`（内容为保存前原文件）；保存成功后状态栏显示备份文件名；备份失败或产物验证失败时目标文件不变；
...
- DOCX 新建、另存为、重命名、移动、删除与工作区 DOCX 正文搜索；
...
点击文件树中的普通 `.docx`，中央区域先显示加载状态，随后以唯一标签打开 Tiptap/ProseMirror 富文本编辑器；使用标签上方的格式工具栏或 `Ctrl+S` 完成基础格式编辑与保存。含图片、表格、页眉页脚、批注或修订等不受支持内容的文档会显示兼容性警告：`degraded` 文档需先点击"确认继续编辑并保存"（确认绑定打开时的 revision）才能编辑与保存；`read-only` 文档只读展示。保存前会自动生成同目录滚动备份 `<文件名>.wenshu.bak`，保存成功后状态栏显示备份文件名；保存失败、产物验证失败或备份失败时原文件不被修改。用其他程序（如 Microsoft Word）修改已打开的 DOCX 后保存会显示外部冲突，本地编辑保留，确认后才重新读取磁盘版本。损坏、加密、伪装、超过 20 MiB 或超出资源预算的 DOCX 会显示稳定错误。
...
│  │  └─ styles/          界面样式
│  └─ shared/             跨进程共享的纯类型契约（含 DOCX 结构化中间模型与纯转换）
├─ tests/                  单元测试与组件行为测试
