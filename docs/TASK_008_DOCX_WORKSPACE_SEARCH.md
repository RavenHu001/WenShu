...
> **状态：已规划，尚未实施。**
>
> 规划日期：2026-08-15。执行时必须从 WP0 开始，逐包完成、逐包验收；在第十一节全部验收项满足前，不得把本任务标记为完成。
>
...
- [ ] 一次查询同时搜索工作区已保存 TXT 与 DOCX；
- [ ] TXT 既有匹配语义完全保持；
- [ ] DOCX 段落、标题、marks 与列表正文可搜索；
- [ ] DOCX 不支持内容不被错误承诺；
- [ ] supported、degraded、read-only 行为确定；
- [ ] 0 字节、损坏、加密、超限和失败文件正确处理；
- [ ] 文件排序、分组、统计、预览与类型标识准确；
- [ ] 未保存正文不参与搜索且 UI 有明确提示。

### 11.2 投影与定位

- [ ] DOCX 正文投影规则有纯测试并与编辑器投影一致；
- [ ] UTF-16、中文、emoji、空段与嵌套列表映射正确；
- [ ] 未打开、已打开和 loading DOCX 只产生唯一标签；
- [ ] revision、范围、文本和宿主二次校验全部生效；
- [ ] dirty 文本未影响原范围时可定位；
- [ ] 仅格式变化时可定位；
- [ ] 文本/结构/外部 revision 变化时只提示过期；
- [ ] read-only 可定位但不可编辑；
- [ ] degraded 定位不自动确认兼容性；
- [ ] 选区、滚动与焦点只使用公开 API；
- [ ] 同一目标只应用一次，迟到回报不污染新状态；
- [ ] 定位不修改正文、不制造 dirty、不进入撤销历史。

### 11.3 取消、预算与性能

- [ ] 总候选 1000、DOCX 200、单文件 200、总匹配 2000 生效；
- [ ] 总并发不超过 4、DOCX 并发不超过 2；
- [ ] 截断原因与优先级准确；
- [ ] 新搜索取消旧搜索；
- [ ] 工作区切换、窗口销毁和组件卸载清理任务；
- [ ] 取消不提交部分 completed 结果；
- [ ] 接近上限的混合工作区不冻结主界面；
- [ ] 性能和取消响应有实际记录。

### 11.4 Electron 与数据安全

- [ ] renderer 不能传入根路径、绝对路径、glob、解析选项或预算；
- [ ] 不跟随符号链接/junction；
- [ ] TXT/DOCX 候选均复用受控读取与 revision；
- [ ] DOCX ZIP/OOXML 全部资源预算保持；
- [ ] 不访问外部资源、不执行宏或嵌入内容；
- [ ] IPC 与日志不泄漏正文、模型、路径或原始异常；
- [ ] 搜索全过程不创建、修改、删除或重命名文件；
- [ ] 工作区无索引、缓存、备份或临时残留；
- [ ] Electron 隔离和 sandbox 基线不变。

### 11.5 质量与文档

- [ ] Task 1 至 Task 7 全量回归通过；
- [ ] 新增投影、混合搜索、契约、IPC、组件和定位测试；
- [ ] 没有无条件 skip、only 或弱化断言；
- [ ] typecheck、lint、format、test、check、build 全部通过；
- [ ] 开发模式与生产构建桌面冒烟通过；
- [ ] Word/WPS 外部修改过期路径完成验证；
- [ ] README、项目基线、测试指南和项目结构同步；
- [ ] 完成报告包含可核对证据；
- [ ] 已知限制与实际行为一致。
