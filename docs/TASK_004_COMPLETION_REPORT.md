...
下一任务已规划为 [TASK-005：多 TXT 标签页与独立编辑会话](./TASK_005_MULTI_TXT_TABS.md)：复用已验证的文档版本（revision）、dirty、保存竞态与“放弃/取消”保护语义，重点实现标签唯一性、切换与关闭、每标签独立编辑器状态、保存并发、确认目标绑定和工作区/窗口聚合保护；不同时加入 DOCX 或工作区全文搜索。
