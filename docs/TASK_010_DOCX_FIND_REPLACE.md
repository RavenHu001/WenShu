...
> **状态：待实施（规划完成，2026-08-19）。**
>
> 本任务从 WP0 开始，逐包实施、逐包验收。只有 WP7 完成，且第十一节全部验收项均有自动测试或手工证据后，才允许把任务标记为已完成。
>
...
- [ ] 活动 DOCX 的 Ctrl+F / Ctrl+H 打开正确侧栏和字段；
- [ ] 查找基于实时未保存 ProseMirror 文档，不读取磁盘；
- [ ] literal、大小写、非重叠、单行和输入长度规则确定；
- [ ] 段落、标题、跨 marks run、列表、中文、emoji 均可匹配；
- [ ] 不跨人工文本块换行匹配；
- [ ] 上一个/下一个循环导航、计数、选区、滚动和焦点正确；
- [ ] 普通/当前匹配装饰可辨识且不修改正文；
- [ ] 2000/2001 匹配预算与截断提示正确；
- [ ] 查找、导航、关闭面板不 dirty、不进入撤销历史。

### 11.2 替换

- [ ] 替换当前项在执行瞬间重新验证实时范围；
- [ ] 空、短、长、中文和 emoji 替换正确；
- [ ] 跨不同 marks run 时继承匹配起点 marks；
- [ ] 段落、标题、列表结构和未匹配格式保持；
- [ ] 全部替换按逆序写入一个 transaction；
- [ ] 全部替换一次 undo/redo 完整恢复/重做；
- [ ] 0 匹配无修改，2001+ 整体拒绝且无部分替换；
- [ ] 模型转换或预算失败 0 dispatch、0 dirty；
- [ ] 替换成功进入 dirty/editRevision，且不自动保存；
- [ ] 保存替换结果继续使用既有 revision、备份和安全替换流程。

### 11.3 兼容性与生命周期

- [ ] read-only 可查不可替换；
- [ ] degraded 未确认可查不可替换，确认绑定当前 revision 后可替换；
- [ ] saving 期间替换不会被旧保存完成结果清除；
- [ ] save-error、conflict、read-error 行为确定；
- [ ] 多 TXT/DOCX 标签状态、装饰、选择和历史隔离；
- [ ] 标签关闭/重开、工作区切换清理完整；
- [ ] 重命名、移动、save-as 后稳定 tabId 搜索状态保持；
- [ ] 工作区结果定位、mutationEpoch 与当前搜索互不污染；
- [ ] 不存在迟到 controls、重复订阅、定时器或 editor 泄漏。

### 11.4 UI、可访问性与安全边界

- [ ] TXT 保持 CodeMirror 既有当前搜索行为；
- [ ] DOCX 面板的输入、按钮、计数、错误、截断和禁用原因准确；
- [ ] 快捷键、焦点进入/恢复、键盘顺序和 screen-reader 名称可用；
- [ ] 当前匹配不只依赖颜色，最小窗口控件仍可达；
- [ ] 不解析正文 DOM、不使用私有 editor API；
- [ ] 不记录查询、替换文本或正文；
- [ ] 无新增 IPC、preload、DesktopApi 或文件系统权限；
- [ ] 无未经评估的新依赖。

### 11.5 质量、性能与文档

- [ ] 纯函数、插件、组件、替换、兼容性、生命周期和回归测试完整；
- [ ] 完整 `check` 通过，跳过项均有合理条件与记录；
- [ ] `build` 通过；
- [ ] 开发和生产 Windows 冒烟通过；
- [ ] 典型与上限夹具性能观察无明显不可接受卡顿；
- [ ] 无控制台未处理异常、明显泄漏或临时残留；
- [ ] README、PROJECT_BASELINE、TESTING 和项目结构已同步；
- [ ] `TASK_010_WP0_REPORT.md` 和 `TASK_010_COMPLETION_REPORT.md` 内容完整；
- [ ] Task 1 至 Task 9 既有能力无回归。
