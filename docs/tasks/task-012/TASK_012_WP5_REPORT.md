...
简体中文 | [English](./TASK_012_WP5_REPORT.en.md)
...
- 项目所有者已于 2026-09-04 在当前 Windows 10 环境手动确认安装、常规使用及删除/卸载流程未出现问题。
  该确认补充自动化安装与外部工作区哈希证据；未被明确报告的 Windows 11、alpha 升级和当前构建 Office
  重测仍按下方限制处理。
- 项目所有者随后确认当前构建的 Word/WPS 基础 DOCX 双向往返、安装版/便携版逐项 GUI 文件流程，以及
  `alpha.0-test → alpha.1` 升级安装均未出现问题。该项为手动验收结论；未提供可复核的 Office/WPS
  版本、操作耗时或屏幕记录。
...
- 当前未安装 WPS，且没有可驱动的真实 Word/WPS GUI 会话，不能补充当前构建 Office 往返的自动化或版本化证据；
  项目所有者已手动确认该流程无问题，既有 Task 7 的 WPS 手工验收也作为历史兼容性证据保留；
- `alpha.0-test → alpha.1` 升级安装已获得项目所有者手动无问题确认；当前工作区没有用于复跑的 alpha.0-test
  安装包；
- 最终安装版/便携版中的 TXT/DOCX 实际打开、编辑、保存、备份、外部冲突、只读/degraded、回收站恢复、
  资源管理器显示与 dirty/saving 关闭保护已获得当前 Windows 10 的手动无问题确认；WP4 E2E 另覆盖 TXT、DOCX、
  备份、About 与 dirty 标签关闭。未提供可逐项复核的最终加固二进制 GUI 操作日志；
- Defender 未运行且 Smart App Control 关闭，未观察到的 SmartScreen/Defender 行为不得概括为其他机器不会提示。

**WP5 门禁：未通过。** 当前 Windows 10 的安装、重装、卸载、外部工作区不变性、中文路径 portable 启动，
以及项目所有者手动确认的升级、最终文件生命周期和 Office/WPS 往返均有证据；但 Windows 11 x64
矩阵仍由项目所有者暂缓，无法满足原始双系统门禁。
