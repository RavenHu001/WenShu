...
> 以下条件当前保持未勾选。只有 WP8 验收并形成完成报告后才能改为 `[x]`。

### 11.1 新建与另存为

- [ ] 可在根或普通子目录新建 TXT、有效 DOCX 和文件夹；
- [ ] 新建冲突不覆盖、不自动改名；
- [ ] 新建 DOCX 可重新导入、打开、编辑和保存；
- [ ] TXT 另存为保持 BOM/换行和保存期间继续编辑语义；
- [ ] DOCX 另存为保持兼容性确认、导出验证与目标备份语义；
- [ ] 覆盖确认绑定目标 revision，目标变化必须重新确认；
- [ ] 另存为成功后源文件不变、当前标签迁移到目标；
- [ ] 失败时标签仍指向源路径且正文/模型不丢失。
...
- [ ] Windows 只改大小写重命名可用；
- [ ] 目标存在不覆盖、不合并；
- [ ] 工作区根、link、other、内部文件和目录移入后代均拒绝；
- [ ] DOCX 伴随备份按规则迁移，失败可回滚或明确 partial failure；
- [ ] 文件和非空目录删除进入 Windows 回收站；
- [ ] dirty 删除需确认，saving 删除被阻止；
- [ ] 删除失败不提前关闭标签；
- [ ] 文件、目录和工作区可在资源管理器中显示且不开放通用 shell。

### 11.3 标签、编辑器与生命周期

- [ ] stable tabId 与 relativePath 已解耦；
- [ ] 同一路径仍只有一个标签；
- [ ] 文件/目录路径迁移同步全部受影响标签且前缀边界准确；
- [ ] TXT 光标、选区、滚动、撤销历史和查找状态保留；
- [ ] DOCX 编辑器实例、选区、滚动、撤销历史和工具栏绑定保留；
- [ ] dirty、saving、editRevision、读取/保存身份和标签顺序保持；
- [ ] 迁移后保存只写新路径；
- [ ] 工作区切换、关闭标签/窗口和文件操作确认不互相污染。

### 11.4 路径、IPC 与数据安全

- [ ] renderer 不能提交根、绝对路径、任意 shell 或危险开关；
- [ ] Windows 非法/保留名称、路径穿越、ADS 和越界确定拒绝；
- [ ] 源和目标任一路径段不跟随 symlink/junction；
- [ ] 主进程写操作串行且发布前复验；
- [ ] 临时写入排他、同目录、sync/close 后发布，失败尽力清理；
- [ ] DOCX 产物验证与覆盖前备份不减少；
- [ ] 删除不调用永久删除 API；
- [ ] IPC/日志不泄漏正文、模型、绝对路径、临时名或原始异常；
- [ ] partial failure 不被伪装成普通成功或普通失败。

### 11.5 工作区、搜索、质量与文档

- [ ] 成功 mutation 刷新工作区并更新选择/展开状态；
- [ ] 成功 mutation 取消活动搜索、清空旧结果与定位；
- [ ] 失败、取消和 reveal 不错误使有效结果失效；
- [ ] Task 1 至 Task 8 全量回归通过；
- [ ] 第 8 节新增测试完整，无 only 或无条件 skip；
- [ ] typecheck、lint、format、test、check、build 全部通过；
- [ ] 开发模式与生产构建 Windows 冒烟通过；
- [ ] 回收站恢复、外部占用和只改大小写具有实际证据；
- [ ] 工作区无临时残留，备份位置符合规则；
- [ ] README、项目基线、测试指南、项目结构和完成报告同步。
