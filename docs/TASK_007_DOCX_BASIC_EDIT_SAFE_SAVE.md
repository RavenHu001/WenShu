...
- `read-only`：结构无法安全映射、文档受保护/加密或存在高风险复杂内容，只读显示可提取正文或错误页；
- `rejected`：不是合法 DOCX、资源超限、关键部件缺失或解析失败，不创建可编辑标签正文。
...
Task 7 完成后，建议下一任务实现“工作区 DOCX 正文搜索与结果定位”：复用 Task 6 已验证的搜索请求、取消、预算、统计和过期结果原则，但为 DOCX 单独设计纯文本提取、段落位置映射、结果 revision 和富文本编辑器定位，不把 DOCX 当作 UTF-8 TXT 读取。
