...
- [ ] 文件树可以选择普通 `.docx`，目录、链接、`.docm` 和其他类型不可读取；
- [ ] 普通段落、标题、marks、字号、颜色、列表和对齐按兼容矩阵导入；
- [ ] 中文、英文、emoji、空段落和多段落正确；
- [ ] 普通 DOCX 数秒内打开且界面不失去响应；
- [ ] 损坏、加密、伪装、超限和资源预算超限返回稳定错误；
- [ ] 复杂文档返回确定兼容性等级和警告；
- [ ] `degraded` 需 revision 绑定确认，`read-only` 不可编辑保存；
- [ ] 不读取网络关系、不执行宏或嵌入内容。

### 11.2 编辑器与多标签验收

- [ ] 基础段落、标题、粗体、斜体、下划线、字号、颜色、列表和对齐可编辑；
- [ ] 复制、剪切、粘贴、撤销和重做可用；
- [ ] 无实际变化不产生 dirty，编辑后产生 dirty；
- [ ] TXT/DOCX 混合标签路径唯一、顺序、激活和关闭正确；
- [ ] 每个 DOCX 标签的模型、选择、滚动、历史、dirty 和错误独立；
- [ ] 普通切换不丢历史、不重置模型、不重复读取；
- [ ] 关闭或工作区切换清理对应富文本会话；
- [ ] TXT CodeMirror、多标签和搜索定位行为无回归。

### 11.3 保存与数据安全验收

- [ ] 保存前校验 expectedRevision，外部变化默认不覆盖；
- [ ] 每次覆盖前成功创建 `<文件名>.wenshu.bak`，内容为保存前原文件；
- [ ] 备份失败时保存中止，原文件与 dirty 保留；
- [ ] 生成产物在替换前通过大小、OOXML 结构和重新导入验证；
- [ ] 临时写入执行完整写入、sync、close 和同目录安全替换；
- [ ] 替换前再次检测保存期间的外部变化；
- [ ] 任一失败点都不先删除、不截断或破坏原文件；
- [ ] 保存期间继续编辑不丢后续修改，不错误清除 dirty；
- [ ] 保存成功返回新 revision，文枢与外部 Office 均可重新打开；
- [ ] dirty 关闭、工作区切换、窗口关闭和保存中保护完整。

### 11.4 Electron 与安全验收

- [ ] DOCX 路径只来自当前主进程工作区会话与规范相对路径；
- [ ] 每次读写都重新执行边界、链接、真实路径、类型和大小校验；
- [ ] IPC 请求字段白名单和模型运行时预算完整；
- [ ] renderer 不拥有 Node.js、文件系统、ZIP、OOXML 或任意 IPC 能力；
- [ ] `nodeIntegration: false`、`contextIsolation: true` 和 sandbox 保持；
- [ ] DOCX 输入被视为不可信内容且不会触发网络、脚本、宏或嵌入对象；
- [ ] 错误、日志和跨进程结果不泄漏正文、XML、绝对路径、Buffer、句柄或调用栈；
- [ ] 工作区除目标 DOCX、滚动备份和保存期间临时文件外没有新增持久产物。

### 11.5 质量验收

- [ ] 第 8 节模型、转换、安全读取、保存、备份、IPC、状态和组件测试完整；
- [ ] Task 1 至 Task 6 既有测试实际执行并通过；
- [ ] 条件跳过仅限有说明且有确定性 mock 覆盖的环境能力；
- [ ] `typecheck`、`lint`、`format:check`、`test` 和完整 `check` 通过；
- [ ] `build` 通过；
- [ ] 开发模式与生产构建桌面冒烟完成；
- [ ] 至少一种外部 Office 程序完成导出打开与外部冲突验证；
- [ ] 备份恢复、保存失败保留原文件和临时文件清理已手工验证；
- [ ] README、项目基线、测试指南、任务文档和完成报告与实际行为一致；
- [ ] 未加入第九节明确排除的功能。
