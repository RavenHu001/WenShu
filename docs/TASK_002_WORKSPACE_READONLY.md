...
- 前置任务：[TASK-001：建立可运行、可测试的桌面应用工程骨架](./TASK_001_PROJECT_BOOTSTRAP.md)
- 后续直接任务：TXT 文件读取与单标签打开
...
- 编写 `docs/TASK_002_COMPLETION_REPORT.md`；
...
- [ ] `nodeIntegration: false`、`contextIsolation: true` 和 `sandbox: true` 保持不变；
- [ ] 渲染进程没有直接导入 Node.js 或 Electron 文件系统能力；
- [ ] preload 未暴露 `ipcRenderer` 或通用 `invoke`；
- [ ] IPC 处理器不接受渲染进程传入的绝对路径；
- [ ] 只有固定的工作区打开和刷新能力进入渲染进程；
- [ ] 跨进程结果不包含原始 `Error` 或 Node.js 对象；
- [ ] 不存在任何用户文件写入操作。

### 11.3 质量验收

- [ ] 新增扫描器测试覆盖第 8.1 节关键分支；
- [ ] 新增界面测试覆盖第 8.2 节关键状态；
...
> 阅读 `docs/PROJECT_BASELINE.md`、`docs/TASK_001_COMPLETION_REPORT.md`、`docs/TASK_002_WORKSPACE_READONLY.md` 以及与当前工作包直接相关的源码。只实现 TASK-002 的 WPx，不实现后续工作包，不进行无关重构，不添加任务范围外功能。严格保持 Electron 安全边界，不暴露通用 IPC、任意路径读取或文件写入能力。修改后运行本工作包要求的测试、`.\scripts\npm.cmd run check` 和 `.\scripts\npm.cmd run build`。最终报告修改文件、关键决策、命令结果、未解决问题和是否满足当前工作包门禁。
...
Task 2 完成后，建议下一任务继续采用小型纵向切片：从只读文件树选择一个 UTF-8 TXT 文件，通过新的受控 IPC 读取正文，并在中央区域的单个只读标签页中显示。
